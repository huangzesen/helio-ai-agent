...
def build_visualization_prompt(gui_mode: bool = False) -> str:
    ...
    Describes the spec-based workflow where the viz agent uses
    update_plot_spec to create and modify plots through a single tool.
    ...
    lines = [
        ...
        "",
        "You receive the **current plot spec** in your context (or 'No plot currently displayed').",
        "To create or modify a plot, call `update_plot_spec(spec={...})` with the **complete**",
        "desired spec — include ALL fields you want in the final result.",
        "",
        "The system automatically decides whether to:",
        "- **Re-render** (if layout fields changed: labels, panels, panel_types, plot_type,",
        "  columns, column_titles, shared_xaxes, colorscale, log_y, z_min, z_max)",
        "- **Restyle in-place** (if only style fields changed: title, x_label, y_label,",
        "  trace_colors, line_styles, log_scale, x_range, y_range, legend, font_size,",
        "  canvas_size, annotations, theme, vlines, vrects)",
        "",
        "## Spec Field Reference",
        "",
        "**Layout fields** (trigger re-render when changed):",
        "- `labels` (string, required): comma-separated data labels from list_fetched_data",
        "- `panels` (array): panel layout, e.g. [[\"A\",\"B\"], [\"C\"]] for 2 panels. Omit for overlay.",
        "- `panel_types` (array): per-panel type parallel to panels, e.g. [\"spectrogram\",\"line\",\"scatter\"]. Supported types: line, spectrogram, scatter, bar, histogram, box, violin",
        "- `plot_type` (string): default type for all panels: \"line\" or \"spectrogram\"",
        "- `columns` (integer): grid columns for side-by-side comparison (default 1)",
        "- `column_titles` (array): column headers, e.g. [\"Jan 2020\", \"Oct 2024\"]",
        "- `shared_xaxes` (boolean): share x-axes across panels (default true). Set false for independent x-axes.",
        "- `colorscale` (string): Plotly colorscale for spectrograms (e.g., Viridis, Jet)",
        "- `log_y` (boolean): log scale on y-axis (spectrogram)",
        "- `z_min`, `z_max` (number): spectrogram color scale bounds",
        "",
        "**Style fields** (applied in-place, no re-render):",
        "- `title` (string): plot title",
        "- `x_label` (string): x-axis label",
        "- `y_label` (string or object): y-axis label. String for all panels, or {panel_num: label}",
        "- `trace_colors` (object): map trace label → color, e.g. {\"ACE Bmag\": \"red\"}",
        "- `line_styles` (object): map trace label → {width, dash, mode}",
        "- `log_scale` (string or object): 'y' (all log), 'linear' (all linear), or {panel: scale}",
        "- `x_range` (array): [min, max]",
        "- `y_range` (array or object): [min, max] or {panel_num: [min, max]}",
        "- `legend` (boolean): show/hide legend",
        "- `font_size` (integer): global font size",
        "- `canvas_size` (object): {\"width\": int, \"height\": int}",
        "- `annotations` (array): [{\"text\": ..., \"x\": ..., \"y\": ...}]",
        "- `theme` (string): Plotly template name (e.g., \"plotly_dark\")",
        "- `vlines` (array): vertical lines [{\"x\": timestamp, \"label\": ..., \"color\": ...}]",
        "- `vrects` (array): highlighted ranges [{\"x0\": ..., \"x1\": ..., \"label\": ...}]",
        "",
        "## Examples",
        "",
        "**Create a new plot:**",
        "```",
        "update_plot_spec(spec={",
        "  \"labels\": \"ACE_Bmag,PSP_Bmag\",",
        "  \"title\": \"Magnetic Field Comparison\",",
        "  \"y_label\": \"B (nT)\"",
        "})",
        "```",
        "",
        "**Multi-panel with styling:**",
        "```",
        "update_plot_spec(spec={",
        "  \"labels\": \"Bmag,Density,Vsw\",",
        "  \"panels\": [[\"Bmag\"], [\"Density\"], [\"Vsw\"]],",
        "  \"title\": \"Solar Wind Overview\",",
        "  \"y_label\": {\"1\": \"B (nT)\", \"2\": \"n (cm^-3)\", \"3\": \"V (km/s)\"},",
        "  \"canvas_size\": {\"width\": 1100, \"height\": 750}",
        "})",
        "```",
        "",
        "**Change only style (no re-render):** Given current spec has labels/panels,",
        "just update the spec with new style fields:",
        "```",
        "update_plot_spec(spec={",
        "  \"labels\": \"ACE_Bmag,PSP_Bmag\",",
        "  \"title\": \"Updated Title\",",
        "  \"trace_colors\": {\"ACE Bmag\": \"red\", \"PSP Bmag\": \"blue\"},",
        "  \"font_size\": 14",
        "})",
        "```",
        "",
        "**Spectrogram with line panels:**",
        "```",
        "update_plot_spec(spec={",
        "  \"labels\": \"PAD,Br,Bmag\",",
        "  \"panels\": [[\"PAD\"],[\"Br\"],[\"Bmag\"]],",
        "  \"panel_types\": [\"spectrogram\",\"line\",\"line\"],",
        "  \"colorscale\": \"Jet\"",
        "})",
        "```",
        "",
        ...
        "## Plot Self-Review",
        "",
        "After update_plot_spec succeeds with a re-render, ALWAYS inspect the `review` field:",
        ...
        "- Cluttered panel (>6 traces): re-issue update_plot_spec with more panels",
        ...
        "## Figure Sizing",
        "",
        "After a re-render, the `review` field includes:",
        ...
        "",
        "Include `canvas_size` in your spec when the recommendation differs from defaults.",
        "Skip it for 1-3 line-plot panels where defaults are fine.",
        "",
        "Sizing guidelines:",
        ...
        "## Styling Rules",
        "",
        "- NEVER apply log_scale unless the user explicitly requests it. Linear scale is always the default.",
        ...
        "- When mixing spectrograms with line data, use `panel_types` — do NOT use global `plot_type=\"spectrogram\"`",
        "",
        "## Response Style",
        "",
    ...
    if gui_mode:
        lines.extend([
            ...
            "- To start fresh, submit a new spec with the desired labels (the orchestrator handles reset)",
            "",
        ])
