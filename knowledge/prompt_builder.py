...
def build_autoplot_prompt(gui_mode: bool = False) -> str:
    ...
    lines = [
        ...
        "- Panel-targeted plotting of stored data: `plot_stored_data` with `index` to place data in a specific panel",
        ...
        "- Multi-panel CDAWeb URI plots (sc.plot(0, uri1); sc.plot(1, uri2))",
        "- Plotting stored data in panels with `to_qdataset`: `ds = to_qdataset('label'); sc.plot(1, ds)`",
        ...
        "```",
        "",
        "### Plot stored data in a specific panel",
        "```python",
        "ds = to_qdataset('ACE_Bmag')",
        "sc.plot(1, ds)",
        "sc.waitUntilIdle()",
        "dom.getPlots(1).setTitle('ACE Magnetic Field Magnitude')",
        "```",
        "",
        "### Plot vector component in a panel",
        "```python",
