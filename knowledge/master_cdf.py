...
def download_master_cdf(dataset_id: str, cache_dir: Path | None = None) -> Path:
    ...
    Args:
        dataset_id: CDAWeb dataset ID (e.g., "AC_H2_MFI").
        cache_dir: Override cache directory (default: ~/.helio-agent/master_cdfs/).
    ...
    logger.debug("Master CDF downloaded: %s (%.1f KB)", filename, size_kb)
...
def fetch_dataset_metadata_from_master(
    ...
    try:
        ...
        logger.debug("Master CDF failed for %s: %s", dataset_id, e)
        return None
