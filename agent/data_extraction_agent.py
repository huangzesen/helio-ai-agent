...
class DataExtractionAgent:
    ...
    def __init__(
        ...
        # Build data-extraction-focused system prompt
        self.system_prompt = build_data_extraction_prompt()

        # Build function declarations (extraction + document + conversation tools)
    ...
    def process_request(self, user_message: str) -> str:
        """Process a data extraction request conversationally.
        ...
        Args:
            user_message: The data extraction request.

        Returns:
            The text response from Gemini after processing.
        """
        self.logger.debug(f"[DataExtraction Agent] Processing: {user_message[:80]}...")
        ...
        try:
            ...
            while iteration < max_iterations:
                ...
                if call_keys and call_keys.issubset(previous_calls):
                    self.logger.debug("[DataExtraction Agent] Duplicate tool call detected, stopping")
                    break
                ...
                for fc in function_calls:
                    ...
                    self.logger.debug(f"[DataExtraction Agent] Tool: {tool_name}({tool_args})")

                    result = self.tool_executor(tool_name, tool_args)

                    if result.get("status") == "error":
                        self.logger.warning(f"[DataExtraction Agent] Tool error: {result.get('message', '')}")
                ...
                self.logger.debug(f"[DataExtraction Agent] Sending {len(function_responses)} tool result(s) back...")
            ...
            log_error(
                "DataExtraction agent request failed",
                exc=e,
                context={"request": user_message[:200]}
            )
            self.logger.warning(f"[DataExtraction Agent] Failed: {e}")
            return f"Error processing data extraction request: {e}"

    def execute_task(self, task: Task) -> str:
        """Execute a single data extraction task.
        ...
        self.logger.debug(f"[DataExtraction Agent] Executing: {task.description}")
        ...
        try:
            ...
            self.logger.debug(f"[DataExtraction Agent] Completed: {task.description}")
            ...
            task.error = str(e)
            log_error(
                "DataExtraction agent task failed",
                exc=e,
                context={"task": task.description}
            )
            self.logger.warning(f"[DataExtraction Agent] Failed: {task.description} - {e}")
            return f"Error: {e}"
