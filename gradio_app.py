...
EXAMPLES = [
    ...
    {"text": "Fetch PSP SPAN-A electron pitch angle distribution data (PSP_SWP_SPA_SF0_L3_PAD) for November 20-25, 2024. I want a 3-panel figure: (1) top panel — electron energy spectrogram showing differential energy flux vs energy with log energy axis and log color scale, (2) middle panel — pitch angle distribution at a mid-range energy level as a spectrogram with pitch angle (0-180°) on the y-axis, (3) bottom panel — magnetic field magnitude computed from the MAGF_SC vector in the same dataset, plotted as a line. Use Plasma colorscale for both spectrograms."},
    {"text": "Fetch PSP SPAN-A electron differential energy flux (dataset PSP_SWP_SPA_SF1_L2_32E, parameter EFLUX_VS_ENERGY) for December 15-20, 2023 and plot it as a spectrogram with log energy axis and log color scale using Jet colorscale. Also fetch PSP magnetic field data for the same period and add the field magnitude as a line plot on a new panel below the spectrogram. Title the plot 'PSP Encounter 17: Electron Energy Spectrogram — Dec 2023' and add a vertical red dashed line at December 17, 2023 12:00 UTC labeled 'Perihelion'."},
    {"text": "Fetch PSP consolidated electron pitch angle distribution (PSP_SWP_SPE_SF0_L3_PAD) for December 23-26, 2024 — this covers PSP's closest perihelion. Create a 4-panel stacked figure: (1) electron energy spectrogram from EFLUX_VS_PA_E_byE_atP with log axes on both y and color, Viridis colorscale, (2) pitch angle distribution spectrogram from EFLUX_VS_PA_E_atE_byP at a representative energy — flux vs pitch angle (0-180°), Viridis with log color, (3) compute the field-aligned electron flux anisotropy — ratio of flux at smallest pitch angle bin to flux at the 90° bin — plot as a line, (4) compute magnetic field magnitude from MAGF_SC and plot as a line. Title: 'PSP Perihelion Dec 2024: Electron Distribution Analysis'. Add a vertical red dashed line at December 24, 2024 12:00 UTC labeled 'Closest Approach'."},
]
