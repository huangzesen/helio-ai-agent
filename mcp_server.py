"""
MCP server entry point for helio-agent.
...
to use helio-agent as a data/visualization specialist.
...
# ---------------------------------------------------------------------------
_parser = argparse.ArgumentParser(description="helio-agent MCP server")
...
mcp = FastMCP(
    "helio-agent",
    instructions=(
        "Helio-agent is an AI-powered natural language interface for spacecraft "
        "and heliophysics data. Use the 'chat' tool to send natural language "
        "requests about spacecraft data visualization and analysis. The agent "
        "supports 52 spacecraft missions from NASA CDAWeb including PSP, Solar "
...
def chat(message: str) -> list:
    """Send a natural language message to the helio-agent and get a response.

    The agent can search spacecraft datasets, fetch data, compute derived
...
@mcp.tool()
def get_status() -> str:
    """Get the current status of the helio-agent session.
    ...
    lines.append(
        ...
        f" / {usage['thinking_tokens']} thinking ({usage['api_calls']} API calls)"
    )
