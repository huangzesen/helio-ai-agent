...
1. **Agent layer** (`agent/`) — LLM with function calling (default: Gemini, via `agent/llm/` adapter layer). Five agent types:
   - `core.py` **OrchestratorAgent** — routes to sub-agents, handles data ops directly. Tools defined in `tools.py` (30 tool schemas).
   - `mission_agent.py` **MissionAgent** — per-spacecraft data specialists (discovery + data_ops tools only).
   - `visualization_agent.py` **VisualizationAgent** — visualization specialist using `render_plotly_json` (Plotly figure JSON with data_label placeholders) + `manage_plot` + `list_fetched_data`.
...
Data flows: User input → LLM function calling (via adapter) → tool execution → result fed back to LLM → natural language response. For computed data: fetch → compute → plot through Plotly renderer.

## Key Technologies

- **Python 3** with virtualenv
- **LLM backend** — abstracted via `agent/llm/` adapter layer. Currently: Google Gemini (`google-genai`). Future: OpenAI, Anthropic, DeepSeek, etc.
...
- **Mission override system**: Learned knowledge (caveats, notes, corrections) is persisted as sparse JSON patches in `{data_dir}/mission_overrides/`. Two levels: mission-level (`{stem}.json`) and dataset-level (`{stem}/{dataset_id}.json`). Generic recursive deep-merge at load time — dicts merge recursively, everything else replaces. No allowlist. Write helpers: `update_mission_override(cache_key, patch)` and `update_dataset_override(dataset_id, patch)` use read-modify-write pattern. See `docs/mission-knowledge-overrides.md` for details.
