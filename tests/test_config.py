...
class TestGetDataDir:
    ...
    def test_default_is_home_helio_agent(self):
        """With no env var or config key, returns ~/.helio-agent."""
        with mock.patch.dict(os.environ, {}, clear=False):
            os.environ.pop("HELIO_AGENT_DIR", None)
            with mock.patch("config.get", return_value=None):
                result = config.get_data_dir()
        assert result == Path.home() / ".helio-agent"

    def test_env_var_overrides_default(self, tmp_path):
        """HELIO_AGENT_DIR env var takes precedence over default."""
        target = tmp_path / "custom-dir"
        with mock.patch.dict(os.environ, {"HELIO_AGENT_DIR": str(target)}):
            result = config.get_data_dir()
    ...
    def test_env_var_beats_config_key(self, tmp_path):
        """HELIO_AGENT_DIR env var has higher priority than config key."""
        env_dir = tmp_path / "env-dir"
        cfg_dir = tmp_path / "cfg-dir"
        with mock.patch.dict(os.environ, {"HELIO_AGENT_DIR": str(env_dir)}):
    ...
    def test_tilde_expansion(self):
        ...
        with mock.patch.dict(os.environ, {"HELIO_AGENT_DIR": "~/my-helio-data"}):
            result = config.get_data_dir()
    ...
    def test_reset_clears_cache(self, tmp_path):
        ...
        dir1 = tmp_path / "dir1"
        dir2 = tmp_path / "dir2"
        with mock.patch.dict(os.environ, {"HELIO_AGENT_DIR": str(dir1)}):
            first = config.get_data_dir()
        assert first == dir1.resolve()

        config._reset_data_dir()
        with mock.patch.dict(os.environ, {"HELIO_AGENT_DIR": str(dir2)}):
            second = config.get_data_dir()
