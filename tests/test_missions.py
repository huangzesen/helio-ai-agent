...
def _all_mission_jsons() -> list[Path]:
    """Discover all mission JSON files on disk."""
    return sorted(_MISSIONS_DIR.glob("*.json"))
...
class TestCatalogDiscovery:
    ...
    def test_all_missions_in_catalog(self):
        ...
        for mission_id in _all_mission_ids():
            assert mission_id in catalog_ids, \
                f"{mission_id} not found in catalog"
        ...
        for sc_id, mission in missions.items():
            ...
            for kw in keywords:
                ...
                if result == sc_id:
                    ...
                    break
            assert found, \
                f"{sc_id} not findable by any of its keywords: {keywords}"
...
class TestKeywordRouting:
    ...
    def test_bare_mission_search_lists_instruments(self):
        ...
        for sc_id in _all_mission_ids():
            ...
            if result is not None and result["spacecraft"] == sc_id:
